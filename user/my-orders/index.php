...
<?php
    ...
    if(!isset($_SESSION["isLogged"])){
        header('Location:'."../?error=Please Login first before go to my-orders!");
    }
?>
   ...
   <?php if ($result->num_rows > 0): ?>
      <?php while($row = $result->fetch_assoc()): ?>
           <tr>
              ...
              <td><?php if($row['payment_method']){
                echo "Online Payment";
              } else{
                echo "Payment On Delivery";
              }              
              ?></td>
              <td><a href="./order_products.php?id=<?php echo $row['order_id']; ?>" class="btn btn-success btn-sm">View</a></td>
            </tr>
      <?php endwhile; ?>
   <?php endif; ?>
